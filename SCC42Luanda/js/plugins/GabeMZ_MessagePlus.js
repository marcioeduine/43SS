 ...
 * 
 * ---------------------------------------------------------------------------
  CÓDIGO para usar na caixa de mensagem do comando de evento SHOW TEXT.
----------------------------------------------------------------------------
O seguinte código deve ser colocado na primeira linha da caixa de mensagem
do comando de evento SHOW TEXT:

    \fs = font size (tamanho da fonte);

    \tgt = target (alvo);

    [p] = party (membro de equipa);

    [targetId] = Id do evento - para seleccionar um evento no mapa (Id do alvo);

    [pTargetId] = o Id da posição actual em que o membro se encontra
    (Id do alvo na formação de equipa);

    [this] = o próprio evento no mapa em que o comando SHOW TEXT está a ser
    colocado (este-esta).
...
\tgt[this]  // a mensagem aparecerá sobre o evento em que o
comando de evento SHOW TEXT está a ser criado;

\tgt[23]    // a mensagem aparecerá sobre o evento número 23 no mapa;
\tgt[-1] ou \tgt[p-1] ou \tgt[p1]  // a mensagem aparecerá sobre o líder da
equipa (player leader);

\tgt[p2]  // a mensagem aparecerá sobre o membro número 2 (party member);
\tgt[p3]  // a mensagem aparecerá sobre o membro número 3 (party member);
\tgt[p4]  // a mensagem aparecerá sobre o membro número 4 (party member);

\tgt[p1]\fs[22] // a mensagem aparecerá sobre o membro número 1
(o líder da equipa), mas o tamanho da fonte de letra mudará para o 22.

O CÓDIGO \fs[number] TAMBÉM PODE SER USADO NA CAIXA DO NOME DA PERSONAGEM
NO COMANDO DE EVENTO SHOW TEXT.

\fs[22]nome da personagem.

O CÓDIGO \tgt[pTargetId] TAMBÉM PODE SER USADO EM BATALHA.

SE ESTÁS A USAR O PLUGIN Akea_BattleSystem.js, TAMBÉM SE PODE USAR O CÓDIGO
\tgt[eIndex], onde "e = enemy; index = posição do inimigo", PARA CRIAR UM
DIÁLOGO PARA O INIMIGO.
